...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{./images/poly/dilated.png}	
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.9\textwidth]{./images/poly/new model.png}	
...
The obtained results are slightly different to \cite{drossos2020sound}: the baseline and dilated perform about 0.1 higher of f1-score in this case, while dessed and proposed model are in line with reference paper but have a limited amount of parameter lower (0.5M vs 0.7M).
...
\begin{itemize}
	\item ROCm \cite{rocm} is an open platform GPU compiler which is capable of running deep learning framework on top of it but my GPU is not supported by this.
	\item PlaidML \cite{Plaid} is a tensor compiler running as back-end of common frameworks. It works well with Keras but has the drawback that Keras does not support back-end different from TensorFlow starting from version 2.4 and it was refactored from version 2.2.5 so PlaidML allows to work with Keras 2.2.4, which is not the leading edge version but all the things needed by this project work well with this project.
	\item DirectML \cite{directML} is a back-end developed by Microsoft which supports TensorFlow on not CUDA GPUs till version 1.15, which is the last version till the big refactor which brought version 2.0. 
\end{itemize}

So both alternatives use old version but the PlaidML one seems more promising since Keras code written for version 2.2.4 is completely usable by newer version: the code has been tested both on local machine with PlaidML as back-end both on Colab with newer Keras version and TensorFlow as back-end.\newline
