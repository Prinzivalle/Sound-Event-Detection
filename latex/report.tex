...
This section contains the practical results achieved training the models and testing them. Both dataset has been divided as follows: 60\% for training, 20\% for validation and the remaining 20\% as test. Model has been trained looking at validation loss: the model with lowest value is saved and the training is stopped when the monitored value has not decreased for longer than 30 epochs. Train is performed with batch size of 16 where possible or 8 if memory is not enough.
